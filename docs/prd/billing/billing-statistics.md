...
| `DEC-billing-statistics-002` | Applied | 支付口径 | 只统计已完结支付尝试；金额按币种分组、任何位置不跨币种相加；按渠道分组；金额以 Herald 快照为准（provider 侧渠道缺可读价时为哨兵 1，CHECK 禁止 0，统计原样累加） | §3.2、§4.1、§5.1、§5.2 | 同上 |
