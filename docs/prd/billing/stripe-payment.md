...
- Stripe 配置管理——通过通用 `realm_config` API（`/api/configs/{realmId}`，ConfigType::Stripe）统一管理，支持 api_key、webhook_secret、publishable_key、timeout、webhook_endpoint_id、async_points_strategy 配置项；base_url 仅用于测试替换 Stripe 服务地址，生产不应覆盖（运维约定，无运行时强制校验）
- 订阅支付处理（周期性计费）
...
- **配置管理规则**：每个 Realm 可配置独立的 Stripe 账户；通过通用 `realm_config` API（`/api/configs/{realmId}`，ConfigType::Stripe）管理，配置项包括 api_key（Secret Key）、webhook_secret（Webhook Signing Secret）、publishable_key（Publishable Key）、timeout（HTTP 请求超时秒数）、webhook_endpoint_id（Webhook 端点 ID，仅作配置记录，当前不参与验签——验签以 webhook_secret 为准）
  - **配置项差异说明**：支持 `async_points_strategy`；`base_url` 仅供测试替换 Stripe 服务地址，生产不应覆盖（运维约定，无运行时强制校验）。Account ID 未作为独立 config_key 实现；Herald 不解析或校验 `sk_test_*` / `sk_live_*` 前缀（密钥原样交给 Stripe，实际环境由 Stripe 密钥本身决定，见 §8.2）；Webhook Endpoint URL 由 `public_base_url` 动态拼接，不作为独立配置项
