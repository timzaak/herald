...
| `POST /api/permission/check` | 管理端批量权限检查（路径无 realm 段，realm 取自登录身份）：**任一**规则命中即 `allowed=true`。仅限自省（RFC 7662 式）：调用者须为已认证用户身份（API Key/CustomUserUi 被 403），且被探测 token 必须属于调用者本人——不可探测他人令牌 |
