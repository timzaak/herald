...
- 用户禁用 TOTP（需通过重新认证：当前密码或任一已绑定强因子）
- 重新生成 TOTP 密钥和备份码（需通过重新认证：当前密码或任一已绑定强因子）
...
- 用户密码系统 — 禁用/重新生成 TOTP 时重新认证（reauth）的密码因素
...
- **用户启用 TOTP**：用户在个人资料 -> Security 页面发起启用，系统生成密钥和二维码，用户扫描后输入验证码确认，完成后显示 10 个备份恢复码（仅显示一次）。已启用状态下重复发起启用返回 409 Conflict（需先禁用再重新启用，不提供原地重启）
...
- 安全约束：验证失败不暴露具体原因；备份码使用后立即失效；密钥重新生成需通过重新认证（当前密码或任一已绑定强因子）
...
- 关键用户路径：启用 TOTP（生成二维码 -> 扫描 -> 验证 -> 显示备份码 -> 确认保存）；登录二次验证（输入验证码或备份码）；禁用/重新生成 TOTP（重新认证确认 -> 操作完成）
- 状态反馈：备份恢复码仅显示一次，需用户确认已保存；验证码错误统一提示；锁定状态显示剩余时间；备份码耗尽提示联系管理员
- 权限可见性：强制模式下禁用按钮不可用；Realm 未启用 TOTP 时用户看不到 TOTP 设置入口
- 异常提示：设置阶段验证码错误/过期可原地重新输入（同一临时 token 内 5 次尝试上限，耗尽后重新发起设置）；登录二次验证阶段任意一次验证码/备份码失败即失效本次临时会话，用户需重新完成登录后再次进入二次验证（失败计数跨尝试累积，连续 5 次锁定 15 分钟）；强制模式未设置用户登录后不强制跳转，登录后访问 Security 页面时看到设置引导横幅
