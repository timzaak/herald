...
- **签名密钥状态**: Active（签名并发布）/ Retained（重叠期只发布）/ Retired（不再发布）— 轮换后旧密钥进入 Retained，重叠期满转 Retired 并从公开公钥集中移除（落库转态在下一次轮换时统一清理；JWKS 发布按保留期时间戳实时过滤，过期即不再发布，两者发布行为等效）
...
- 发现文档与 JWKS 响应带公开缓存头（`Cache-Control: public, max-age=300`）：内容按 Realm/issuer 维度稳定，允许标准客户端与中间层短暂缓存；userinfo 响应禁止缓存（`no-store`）
...
- userinfo 的访问控制以访问令牌有效性 + openid 语义授权为准：状态码区分无效令牌（401）与"令牌有效但未携带 openid 语义"（403），但两类拒绝响应都不携带任何身份资料细节（差异信息不泄露）
