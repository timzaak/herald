...
mod registration_guard {
    /// Guard: every scenarios/*.rs file must be declared as a module in the
    /// mod.rs of its own directory. An unregistered file never compiles or
    /// runs, yet still reads like acceptance coverage and drifts from the
    /// current contract (six such dead files were found and removed).
    ///
    /// The walk is recursive: subdirectories (billing/, points/,
    /// credit_bucket/, legal/) are checked the same way — each .rs file
    /// against its own directory's mod.rs, and each subdirectory itself
    /// against its parent's mod.rs. A non-recursive read_dir left the four
    /// subdirectories as blind spots.
    #[test]
    fn every_scenario_file_is_registered() {
        ...
        while let Some(current) = stack.pop() {
            let mod_rs_path = current.join("mod.rs");
            let mod_rs = std::fs::read_to_string(&mod_rs_path)
                .unwrap_or_else(|_| panic!("read {}", mod_rs_path.display()));
            for entry in std::fs::read_dir(&current)
                .unwrap_or_else(|_| panic!("read {}", current.display()))
                .flatten()
            {
                let path = entry.path();
                if path.is_dir() {
                    let name = path
                        ...
                        .unwrap_or_default()
                        .to_string();
                    // Matches both `mod x;` and `pub mod x;` declarations.
                    if !mod_rs.contains(&format!("mod {name};")) {
                        unregistered.push(format!("{name}/ (directory)"));
                    }
                    stack.push(path);
                    ...
                    if name != "mod" && !mod_rs.contains(&format!("mod {name};")) {
        ...
        assert!(
            ...
            "scenario files not registered in their mod.rs (register or delete them): {unregistered:?}"
        );
    }
}
