...
架构上和 [Google One Tap](/docs/zh/integration/third-party-backend) 同构：接收上游签发的 JWT 凭证 → 服务端校验 → 用户匹配 → 双分支签发。区别只在凭证来源是 Apple 而非 Google，以及邮箱处理规则不同（见下文）。
...
- [第三方后端对接](/docs/zh/integration/third-party-backend) — 下游 Code+PKCE 和 Bearer token 的整体上下文
