...
如果你只是想在 Herald 托管的 auth 页面上换成自己的 logo 和配色——那是 [White-label](/docs/zh/customization/branding)，零代码——这条路径不适合你。如果你已经有后端，希望由后端中转 token、把用户调用代理成一个 BFF——那是 [Third-Party Integration](/docs/zh/integration/third-party-backend)，由你的后端做 OAuth + BFF 并在服务端持有 token——这条路径同样不适合你。
...
- [Third-Party Integration](/docs/zh/integration/third-party-backend)——后端中转的 OAuth + BFF 路径
- [White-label](/docs/zh/customization/branding)——品牌化 Herald 托管 auth 页面，零代码
