...
| `charge.refunded` | Payments → charge.refunded | 退款；充值积分按单笔退款增量比例回收（以剩余额度封顶），累计全额退款时回收一次性购买角色 |
...
- 充值退款按单笔增量回收：每笔退款按 `本笔退款金额 / 原支付金额` 比例削减该笔支付发放的积分，以剩余额度封顶，同一退款单重复推送只回收一次。部分退款保留支付来源角色；累计退款达到原支付金额的那笔清掉剩余积分并回收角色
- 订阅退款立即取消订阅并撤销其配额权益
