...
4. Refunds and cancellations claw the credits back automatically. Subscription entitlements are revoked on any refund; one-time purchase roles are revoked only when the refund covers the whole payment, so partial refunds keep them.
