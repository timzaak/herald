...
| `charge.refunded` | Payments → charge.refunded | Refund; claw back top-up credits per refund (each refund's amount ÷ original payment, capped at what remains); revoke one-time purchase roles when cumulative refunds reach the full amount |
...
- Top-up refunds claw back credits per refund: each refund reduces the credits granted by that payment by `refund amount / original payment`, capped at the remaining balance, and the same refund delivered twice claws back once. Partial refunds keep the payment-granted roles; the refund that brings the cumulative total to the full payment sweeps the remaining credits and revokes them
- Subscription refunds cancel the subscription immediately and revoke its quota entitlements
