...
| `subscription.canceled` | Subscription canceled (immediate or end-of-term); revoke remaining grants and quota entitlements |
...
| Event | Herald handling |
|------|----------------|
| `refund.created` | Refund; claw back top-up credits per refund (each refund's amount ÷ original payment, capped at what remains); revoke one-time purchase roles when cumulative refunds reach the full amount |
