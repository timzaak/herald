...
This is **not** the right path if you only want to put your logo and colors on Herald's hosted auth pages — that is [White-label](/docs/customization/branding), with zero code. It is also **not** the right path if you have a backend that should mediate the token and proxy user calls through a BFF — that is [Third-Party Integration](/docs/integration/third-party-backend), where your backend does OAuth + BFF and holds the token server-side.
...
- [Third-Party Integration](/docs/integration/third-party-backend) — the backend-mediated OAuth + BFF path
- [White-label](/docs/customization/branding) — branding Herald's hosted auth pages, no code
